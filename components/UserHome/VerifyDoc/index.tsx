  ...
  return (
    <div>
      <div css={styles.heading}>
        Upload a Document to Verify it on Blockchain
      </div>
      <Form
        name="basic"
        labelCol={{ span: 8 }}
          ...
          {showState && (
            <>
              {docsFound?.[0][7] ? (
                ...
                <Badge
                  count="NOT FOUND"
                  style={{ backgroundColor: "#FF5733", height: 22 }}
                />
              )}
            </>
          )}
      ...
      {showState && docsFound?.[0][7] && (
        ...
        >
          ...
          <div style={{ padding: "0px 20px", width: "100%" }}>
            <div css={styles.participantHeading}>Participants</div>
            {/* @ts-ignore*/}
            {docsFound?.[1][0].map((val, key) => {
              return (
                <div key={key} css={styles.participantDetails}>
                  <div css={styles.name}>
                    <div>{docsFound?.[1][1][key]}</div>
                    <div>{val}</div>
                  </div>
                  <div css={styles.status}>
                    <Tag color={docsFound?.[1][2][key] ? "green" : "gold"}>
                      {docsFound?.[1][2][key] ? "SIGNED" : "PENDING"}
                    </Tag>
                  </div>
                </div>
              );
